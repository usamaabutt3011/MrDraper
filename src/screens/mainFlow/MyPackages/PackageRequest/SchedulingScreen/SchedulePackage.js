...
class SchedulePackage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isRefresh: false,
    ...
    validate = async () => {
        const { selectedTimeSlot, selectedDay } = this.state;
        const { scheduleQuiz } = this.props;
        var params = scheduleQuiz.scheduleQuizObj
        if (params.package_send_time == "" || selectedTimeSlot == '') {
            Toast.show('Select Time Slot.')
        } else {
            if (params.package_send_date == "" || selectedDay == '') {
                Toast.show('Select Date.')
            } else {
                this.props.navigation.push('DeliveryAddress')
            }
        }
    }
        ...
        return (
            <View style={styles.container}>
                ...
                >
                    ...
                    <View style={styles.subContainerSchdule}>
                        <MediumTitle
                            text={`${yml.q1}`}
...
export default connect(mapStateToProps, mapDispatchToProps)(SchedulePackage);
